<template>
  <q-page class="q-pa-sm bg-white">
          ...
          <q-tab-panels v-model="tab" animated class="bg-white">
            <q-tab-panel name="all" class="q-pa-none full-height" style="max-height: 77vh;">
              <q-list class="" style="min-height: 75vh;">
                ...
                <div style="height: 66vh; overflow: auto;">
                  <q-item-label header class="text-center">{{ accountActiveList.length }} {{ t('uAccounts') }}</q-item-label>
                  <span v-for="(account, index) in accountActiveList" :key="index" @click="selectAccount(account)">
                    <account-item
                      :name="account.name"></account-item>
                  </span>
                </div>
              </q-list>
            </q-tab-panel>
            ...
            <q-tab-panel name="disabled" class="q-pa-none">
              <q-list class="">
                ...
                <div style="height: 66vh; overflow: auto;">
                  <q-item-label header class="text-center">{{ accountDisabledList.length }} {{ t('inactive') }}</q-item-label>
                  <span v-for="(account, index) in accountDisabledList" :key="index" @click="selectAccount(account)">
                    <account-item
                      :name="account.name"></account-item>
                  </span>
                </div>
        ...
        <q-card class="no-shadow" bordered style="min-height: 88vh;">
          <q-toolbar class="text-black ">
            <q-item class="q-subtitle-1 q-pl-md">
              <q-item-section>
                <q-item-label lines="1">{{ selectedAccount.name || t('accounts') }}</q-item-label>
                <q-item-label caption lines="2">
                  <span class="text-weight-bold">{{ selectedAccount.phone1 }}</span>
                </q-item-label>
              </q-item-section>
            </q-item>
            ...
            <div v-if="selectedAccount.id">
              <q-btn round flat icon="delete" color="red" @click="confirmDelete(selectedAccount.id)"
              :disabled="!tablePermissions.account.d"></q-btn>
              <q-btn round flat icon="edit" color="primary" @click="showAdd(selectedAccount.id)"
          ...
          <div v-for="(detail, detailIndex) in detailList" :key="detailIndex">
            ...
            <user-detail-item :icon="detail.icon" :textColor="detail.textColor"
              :value="selectedAccount[detail['field']]" :label="t(detail.label)"></user-detail-item>
      ...
      >
        <q-card class="no-border no-border">
          <q-toolbar class="text-black ">
            <q-item class="q-subtitle-1 q-pl-md">
              <q-item-section>
                <q-item-label lines="1">{{ selectedAccount.name }}</q-item-label>
                <q-item-label caption lines="2">
                  <span class="text-weight-bold">{{ selectedAccount.code }}</span>
                </q-item-label>
              </q-item-section>
            </q-item>
            ...
            <q-btn round flat icon="edit" @click="showAdd(selectedAccount.id)"
            :disabled="!tablePermissions.account.u"/>
            <q-btn round flat icon="keyboard_backspace" @click="selectAccount({})"/>
    ...
    <q-dialog v-model="showAccountForm">
      <account-dialog :accountId="accountId" @closeMeEvent="handleCloseDialog"></account-dialog>
    </q-dialog>
  </q-page>
</template>
...
const AccountItem = defineAsyncComponent(() => import('components/VendorItem.vue'));
...
const accountActiveList = computed({
  get: () => {
    const activeList = accountStore.getAccounts.filter((p) => p.is_active) || [];
    if (search.value) {
